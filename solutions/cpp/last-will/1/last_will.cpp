...
namespace estate_executor{
    int assemble_account_number(int secret_modifier){
        int result = garcia::bank_number_part(secret_modifier) + zhang::bank_number_part(secret_modifier) + khan::bank_number_part(secret_modifier);
        return result;}

    int assemble_code() {
        int reds = zhang::red::code_fragment() + garcia::red::code_fragment() + khan::red::code_fragment();
        int blues = zhang::blue::code_fragment() + garcia::blue::code_fragment() + khan::blue::code_fragment();
        return reds * blues; }
}